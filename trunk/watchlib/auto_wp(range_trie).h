...
namespace Hongyi_WatchPoint {
	...
	public:
		...
		void	range_to_trie	(ADDRESS target_addr, ADDRESS target_size);
		void	trie_to_range	(ADDRESS target_addr, ADDRESS target_size);
		...
		void rm_watchpoint	(ADDRESS target_addr, ADDRESS target_size, FLAGS target_flags, bool wp_operate);
        void invalidate_wlb (ADDRESS target_addr, ADDRESS target_size);
		void add_watchpoint	(ADDRESS target_addr, ADDRESS target_size, FLAGS target_flags, bool wp_operate);
...
namespace Hongyi_WatchPoint{
	...
	void WatchPoint<ADDRESS, FLAGS>::add_watch_wp (ADDRESS target_addr, ADDRESS target_size) {
		...
		add_watchpoint (target_addr, target_size, WA_READ | WA_WRITE, true);
	...
	void WatchPoint<ADDRESS, FLAGS>::add_read_wp (ADDRESS target_addr, ADDRESS target_size) {
		...
		add_watchpoint (target_addr, target_size, WA_READ, true);
		range_data_t temp = range;
		general_fault (target_addr, target_size, (WA_READ | WA_WRITE), trie.top_change, trie.mid_change, trie.bot_change, false, false);
		range = temp;
	...
	void WatchPoint<ADDRESS, FLAGS>::add_write_wp (ADDRESS target_addr, ADDRESS target_size) {
		...
		add_watchpoint (target_addr, target_size, WA_WRITE, true);
		range_data_t temp = range;
		general_fault (target_addr, target_size, (WA_READ | WA_WRITE), trie.top_change, trie.mid_change, trie.bot_change, false, false);
		range = temp;
	...
	void WatchPoint<ADDRESS, FLAGS>::add_watchpoint(ADDRESS target_addr, ADDRESS target_size, FLAGS target_flags, bool wp_operate) {
		...
		if (iter != wp.end() && iter->size == 0) {// This is special for size = 0. It would then check for flag inclusion and split it if necessary.
			...
			if (!flag_inclusion (target_flags, iter->flags) ) {
				if (target_addr == 0) {
					...
					modify_t.flags = target_flags | iter->flags;
					Modify_wp(modify_t, iter, wp_operate);
					iter = Insert_wp(insert_t, iter, wp_operate);
					insert_t.addr = target_addr + target_size;
					insert_t.size = 0 - insert_t.addr;
					insert_t.flags = iter->flags;
					Push_back_wp(insert_t, wp_operate);
		...
		if (wp_operate) {//If add_watchpoint is called by range_to_trie then range operate is handled manully there
			if (addr_covered(target_addr, (*iter) ) ) {
				if (iter->flags & WA_TRIE) {
					...
					if (iter != wp.begin() ) {
						...
						while (previous_iter != wp.begin() && (previous_iter->flags & WA_TRIE) && previous_iter->addr + previous_iter->size == start_addr) {
							start_addr = previous_iter->addr;
							previous_iter--;
						}
					}
					if (previous_iter->addr + previous_iter->size == start_addr && (previous_iter->flags & WA_TRIE) )
					//In case that wp.begin() is also marked as TRIE
						start_addr = previous_iter->addr;
					
					if (target_addr + target_size - 1 < iter->addr + iter->size - 1) {//If the iter is too big
						after_iter = iter + 1;
						while (after_iter != wp.end() && (after_iter->flags & WA_TRIE) && after_iter->addr == end_addr + 1) {
			...
			else if ( (iter->flags & WA_TRIE) && target_addr + target_size - 1 < iter->addr + iter->size - 1) {
				after_iter = iter + 1;
				while (previous_iter != wp.end() ) {
					end_addr = after_iter->addr + after_iter->size - 1;
					after_iter++;
				}
				Range_load(start_iter, end_iter, false);
		...
		if (addr_covered (target_addr, (*iter) ) ) {
			...
			if (iter->addr == target_addr) {
				...
				if (iter != wp.begin() ) {
					...
					if(start_iter->addr + start_iter->size == target_addr && start_iter->flags == (iter->flags | target_flags) ) {
                        ...
                        // Erase old guys.
						iter = Erase_wp(start_iter, wp_operate);
			...
			if (iter != wp.begin()) {
				...
				if (start_iter->addr + start_iter->size == target_addr && start_iter->flags == target_flags) {
					...
					insert_t.flags = target_flags;
					iter = Erase_wp(start_iter, wp_operate); // Erase old guy
				}
				else {
	...
	template <class ADDRESS, class FLAGS>
	void WatchPoint<ADDRESS, FLAGS>::rm_watchpoint (ADDRESS target_addr, ADDRESS target_size, FLAGS target_flags, bool wp_operate) {
		watchpoint_t<ADDRESS, FLAGS> modify_t = {0, 0, 0};
			...
			if (iter->flags & target_flags) {
				...
				if (insert_t.flags) {
					...
					if (previous_iter->addr + previous_iter->size == insert_t.addr && previous_iter->flags == insert_t.flags) {
						...
						Modify_wp(modify_t, previous_iter, wp_operate);
					}
					else {
						iter = Insert_wp(insert_t, iter, wp_operate);
						iter++;
	...
	void WatchPoint<ADDRESS, FLAGS>::rm_watch (ADDRESS target_addr, ADDRESS target_size) {
		...
		rm_watchpoint (target_addr, target_size, (WA_READ | WA_WRITE), true);
		range_data_t temp = range;
		general_fault (target_addr, target_size, (WA_READ | WA_WRITE), trie.top_change, trie.mid_change, trie.bot_change, false, false);
		range = temp;
	...
	void WatchPoint<ADDRESS, FLAGS>::rm_read (ADDRESS target_addr, ADDRESS target_size) {
		...
		rm_watchpoint (target_addr, target_size, WA_READ, true);
		range_data_t temp = range;
		general_fault (target_addr, target_size, (WA_READ | WA_WRITE), trie.top_change, trie.mid_change, trie.bot_change, false, false);
		range = temp;
	...
	void WatchPoint<ADDRESS, FLAGS>::rm_write (ADDRESS target_addr, ADDRESS target_size) {
		...
		rm_watchpoint (target_addr, target_size, WA_WRITE, true);
		range_data_t temp = range;
		general_fault (target_addr, target_size, (WA_READ | WA_WRITE), trie.top_change, trie.mid_change, trie.bot_change, false, false);
		range = temp;
	...
	void WatchPoint<ADDRESS, FLAGS>::range_to_trie	(ADDRESS target_addr, ADDRESS target_size) {
		...
		ADDRESS start_addr;
		ADDRESS end_addr;
		ADDRESS overall_start = target_addr;
		ADDRESS overall_end = target_addr + target_size - 1;

		//starting part
		iter = search_address(target_addr, wp);
		
		if (iter == wp.end() || iter->addr > target_addr + target_size - 1) {
			if (iter == wp.begin() )
				start_addr = 0;
			else
			...
			if (iter->addr > target_addr + target_size - 1)
				end_addr = target_addr + target_size - 1;
			else
				end_addr = -1;
				
			range_cache_iter = Range_load(start_addr, end_addr, false);
			range_cache_iter->start_addr = target_addr;
			range_cache_iter->end_addr = target_addr + target_size - 1;
			...
			if (target_addr + target_size - 1 != end_addr) {
				insert_range.start_addr = target_addr + target_size;
		...
		if (addr_covered(target_addr, (*iter) ) ) {
			if (iter->flags & WA_TRIE) {
				...
				//In case the wp.begin() is also WA_TRIE:
				if (previous_iter == wp.begin() && (previous_iter->flags & WA_TRIE) && previous_iter->addr + previous_iter->size == start_addr)
					start_addr = previous_iter->addr;
				overall_start = start_addr;
				
				end_addr = iter->addr + iter->size - 1;
				iter++;
				while (iter != wp.end() && (iter->flags & WA_TRIE) && iter->addr == end_addr + 1) {
					end_addr = iter->addr + iter->size - 1;
					iter++;
				}
				...
				if (end_addr >= target_addr + target_size - 1) {
					overall_end = end_addr;
					Range_load (overall_start, overall_end, false);
					add_watchpoint (target_addr, target_size, WA_TRIE, false);
					return;
				}
			}
			...
			range_cache_iter = Range_load(start_addr, end_address, false);
			if (target_addr == start_addr)
				range_cache.erase(range_cache_iter);
			else
				range_cache_iter->size = target_addr - start_addr;
	...
	template <class ADDRESS, class FLAGS>
	void WatchPoint<ADDRESS, FLAGS>::trie_to_range	(ADDRESS target_addr, ADDRESS target_size) {
		typename deque<watchpoint_t<ADDRESS, FLAGS> >::iterator iter;
		typename deque<watchpoint_t<ADDRESS, FLAGS> >::iterator previous_iter;
		range_t<ADDRESS>	insert_range;
		typename deque< range_t<ADDRESS> >::iterator	range_cache_iter;
		...
		iter = search_address(target_addr, wp);
		//check merge condition. If merge, then cur_range_num should decrement by one.
		if ( iter != wp.begin() && (iter->flags & WA_TRIE) && iter->addr = target_addr) {
			if ( (iter - 1)->addr + (iter - 1)->size == iter->addr && (iter - 1)->flags == (iter->flags & ~(WA_TRIE) ) )
				range.cur_range_num--;
		}
		//Starting part
		if (iter == wp.end() || iter->addr > target_addr + target_size - 1)
			return;//We only care when there are wp within this range that flagged as "WA_TRIE"
		if (iter->flags & WA_TRIE) {
			if (addr_covered (target_addr, (*iter) ) ) {
				start_addr = iter->addr;
				previous_iter = iter - 1;
			...
			if (start_addr < target_addr) {
				insert_range.start_addr = start_addr;
				insert_range.end_addr = target_addr - 1;
				range_cache.insert(insert_range);
			}
			if (end_addr > target_addr + target_size - 1) {
				insert_range.start_addr = target_addr + target_size;
				insert_range.end_addr = end_addr;
		...
		//check merge condition. If merge, then cur_range_num should decrement by one.
		if ( iter != wp.end() && iter != (wp.end() - 1) && (iter->flags & WA_TRIE) && iter->addr + iter->size - 1 = target_addr + target_size - 1) {
			if ( iter->addr + iter->size == (iter + 1)->addr && (iter + 1)->flags == (iter->flags & ~(WA_TRIE) ) )
				range.cur_range_num--;
		}
		
		rm_watchpoint	(target_addr, target_size, WA_TRIE, false);
		
		//walk through to load the newly modifyed "wp"s in to the range cache
		unsigned long long trash;
		general_fault (target_addr, target_size, (WA_READ | WA_WRITE), trash, trash, trash, false, false);
	...
	typename deque<watchpoint_t<ADDRESS, FLAGS> >::iterator WatchPoint<ADDRESS, FLAGS>::Insert_wp (const watchpoint_t<ADDRESS, FLAGS>& insert_wp, typename deque<watchpoint_t<ADDRESS, FLAGS> >::iterator iter, bool wp_operate) {
		if (wp_operate || (iter->flags & WA_TRIE) || (insert_wp.flags & WA_TRIE) ) {
			ADDRESS		start_addr;
			ADDRESS		end_addr;
			range_t<ADDRESS>	insert_range;
			typename deque< range_t<ADDRESS> >::iterator	range_cache_iter;
	...
	template <class ADDRESS, class FLAGS>
	void WatchPoint<ADDRESS, FLAGS>::Modify_wp (const watchpoint_t<ADDRESS, FLAGS>& modify_t, typename deque<watchpoint_t<ADDRESS, FLAGS> >::iterator iter, bool wp_operate) {
		if (wp_operate || (modify_t.flags & WA_TRIE) || (iter->flags & WA_TRIE) ) {
			ADDRESS		start_addr;
			ADDRESS		end_addr;
			range_t<ADDRESS>	insert_range;
	...
	template <class ADDRESS, class FLAGS>	
	void WatchPoint<ADDRESS, FLAGS>::Push_back_wp (const watchpoint_t<ADDRESS, FLAGS>& insert_wp, bool wp_operate) {
		if (wp_operate || (insert_wp.flags & WA_TRIE) ) {
			ADDRESS		start_addr;
			range_t<ADDRESS>	insert_range;
			typename deque< range_t<ADDRESS> >::iterator	range_cache_iter;
	...
	template <class ADDRESS, class FLAGS>
	typename deque< watchpoint_t<ADDRESS, FLAGS> >::iterator WatchPoint<ADDRESS, FLAGS>::Erase_wp (typename deque<watchpoint_t<ADDRESS, FLAGS> >::iterator iter, bool wp_operate) {
		if (wp_operate || (iter->flags & WA_TRIE) ) {
			ADDRESS		start_addr;
			ADDRESS		end_addr;
