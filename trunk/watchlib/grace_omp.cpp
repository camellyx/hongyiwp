...
VOID Prefetch0Call(THREADID threadid)
{
    ...
    restart_thread_counts(this_thread);

    (this_thread->wp).add_watch_wp(0, MEM_SIZE);
    (this_thread->self_mem_ptr->mem).clear();
...
VOID ThreadStart(THREADID threadid, CONTEXT *ctxt, INT32 flags, VOID *v)
{
    ...
    if (thread_num == 0) {
        ...
        // We ***DO NOT*** increase the child_thread_num of the parent yet.
        // In this OMP Grace system, the parent only has extra children once
        // OpenMP starts up children threads.  We demarcate that with the
...
VOID Prefetch1Call(THREADID threadid)
{
    OS_THREAD_ID this_threadid = PIN_GetTid();
    thread_wp_data_t* this_thread = thread_map[this_threadid];
...
{
    // Killing a thread does not add its data into the big total.  Grace should
    // only be on in parallel sections, and we're placing prefetcht0 at the
    // beginning of all parallel sections, and prefetcht1 at the end.
    OS_THREAD_ID this_threadid = PIN_GetTid();
    thread_wp_data_t* this_thread = thread_map[this_threadid];
    OS_THREAD_ID this_parent_threadid = this_thread->parent_threadid;
    if (!this_thread->root) {
...
VOID Instruction(INS ins, VOID *v)
{
    ...
    for (UINT32 memOp = 0; memOp < memOperands; memOp++)
    {
        ...
        {
            INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)Prefetch0Call, IARG_THREAD_ID, IARG_END);
            INS_Delete(ins);
        }
        else if (INS_Opcode(ins) == XED_ICLASS_PREFETCHT1)
        {
            INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)Prefetch1Call, IARG_THREAD_ID, IARG_END);
            INS_Delete(ins);
        }
